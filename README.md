...
- 使用独立睡眠/唤醒/正面蹲坐资源、互动图集和 16 方向视线图集；单击唤醒采用 8×6、48 个完整猫姿态，在约 2 秒内依次完成缓慢睁眼、前爪伸展、高臀伸腰、一次哈欠、收爪低伏和连续起身。睡眠首帧与标准蹲坐末帧均为像素级硬锚点，状态交界不再改变爆米花的视觉尺度。
- 渲染始终只显示一张完整精灵，不进行透明叠化；伸展与双击互动均使用真实连续中间姿态帧。双击互动使用通过质量门禁的 8×6、48 帧图集按行顺序一次性播放，完成后由同一动画 completion 收束到蹲坐，不存在并行的站立计时器或循环动画。
- 睡眠呼吸以显示器同步频率连续微幅形变呈现；视线角度经过低通、滞后防抖并以最多 30 次/秒逐方向切换，回到脸前方时先经过近正面方向再显示标准蹲坐。
